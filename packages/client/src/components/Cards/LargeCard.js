...
const CardWrapper = styled(MainCard)(({ theme, backgroundColor }) => ({
  backgroundColor: theme.palette.mode === 'light' ? theme.palette[backgroundColor].dark : theme.palette.dark.dark,
  color: '#fff',
  overflow: 'hidden',
  position: 'relative',
  '&:after': {
              ...
              <Grid item sx={{ mb: 1.25 }}>
                <Typography
                  sx={{
                    fontSize: '1rem',
                    fontWeight: 500,
                    color: theme.palette.mode === 'light' ? theme.palette[backgroundColor][200] : '#8492c4'
                  }}
                >
                  {subtitle}
                </Typography>
